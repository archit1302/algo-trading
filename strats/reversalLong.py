...
class ReversalLongStrategy(BaseStrategy):
    ...
    def fetch_data(self, symbol: str):
        try:
            ...
            self.logger.error(f"Error fetching {symbol} for ReversalLong: {e}")
            return None
    ...
    def generate_signal(self, symbol: str, df):
        ...
        trigger_bar      = df.iloc[-3]     # bar where price dipped/reversed outside lower BB
        confirmation_bar = df.iloc[-2]     # next bar for confirmation
        entry_open       = df.iloc[-1]['open']

        # ==== ENTRY conditions ====
        # 1) trigger bar low < lower BB and close > lower BB
        cond1 = (trigger_bar['low']   < trigger_bar['bb_lower']) and \
                (trigger_bar['close'] > trigger_bar['bb_lower'])
        # 2) confirmation bar close & low both above lower BB (inside band)
        cond2 = (confirmation_bar['close'] > trigger_bar['bb_lower']) and \
                (confirmation_bar['low']   > trigger_bar['bb_lower'])
        # 3) BB width % > 1% on trigger bar
        width_pct = ((trigger_bar['bb_upper'] - trigger_bar['bb_lower']) 
                     / trigger_bar['bb_lower']) * 100
        cond3 = width_pct > 1
        # 4) RSI on confirmation bar < 40
        cond4 = confirmation_bar['rsi'] < 40
