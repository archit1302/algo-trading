...
class EMAHTFLongStrategy(BaseStrategy):
    ...
    def generate_signal(self, symbol: str, df):
        ...
        follow_bar = df.iloc[-2]                 # next bar: must close above cross_bar.high
        entry_open = df.iloc[-1]['open']         # price for entry/exit on this bar’s open
        
        # ==== ENTRY conditions ====
        # 1) cross_bar open below 50 EMA & close above 50 EMA
        crossed_50 = (cross_bar['open'] < cross_bar['ema50']) and (cross_bar['close'] > cross_bar['ema50'])
        # 2) entire setup above 200 EMA
        above_200  = (cross_bar['close'] > cross_bar['ema200']) and (follow_bar['close'] > follow_bar['ema200'])
        # 3) follow-through bar closes above high of cross_bar
        follow_up  = follow_bar['close'] > cross_bar['high']

        if crossed_50 and above_200 and follow_up:
            return {
                'strategy':        self.name,
                'symbol':          symbol,
                'price':           entry_open,
