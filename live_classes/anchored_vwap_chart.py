...
def create_advanced_chart(df, anchor_date, file_path, debug_mode=False):
    """Create an advanced chart similar to a trading platform view"""
    # Filter out rows where AVWAP is NaN
    df_plot = df.dropna(subset=['anchored_vwap'])
    
    if len(df_plot) == 0:
        print("No VWAP data available for plotting.")
        return None
    
    # Create figure with 2 subplots (1 for price + VWAP, 1 for volume)
    ...
    for i, row in df_plot.iterrows():
        ...
        # Plot candle wicks (high-low range)
        ax1.plot([row['datetime'], row['datetime']], [row['Low'], row['High']], 
                color=color, alpha=alpha, linewidth=1.5)
        
        # Plot candle body (open-close range)
        body_height = abs(row['Open'] - row['Close'])
        body_bottom = min(row['Open'], row['Close'])
        ax1.bar(row['datetime'], body_height, bottom=body_bottom, 
               color=color, alpha=alpha, width=0.0003)
    
    # Plot Anchored VWAP
    ax1.plot(df_plot['datetime'], df_plot['anchored_vwap'], 
            color='purple', linewidth=2, label='Anchored VWAP')
    
    # Highlight crossover points
    crossover_above = df_plot[df_plot['Crossover_Above_VWAP'] == True]
    crossover_below = df_plot[df_plot['Crossover_Below_VWAP'] == True]
    
    ax1.scatter(crossover_above['datetime'], crossover_above['Close'], 
               color='lime', marker='^', s=120, label='Crossover Above VWAP')
    
    ax1.scatter(crossover_below['datetime'], crossover_below['Close'], 
               color='red', marker='v', s=120, label='Crossover Below VWAP')
    
    # Mark anchor point with a vertical line
    if anchor_date in df_plot['datetime'].values:
        ax1.axvline(x=anchor_date, color='darkblue', linestyle='--', 
                   linewidth=1.5, label=f'Anchor Point ({anchor_date.strftime("%H:%M")})')
    
    # Plot volume in the bottom subplot
    ax2.bar(df_plot['datetime'], df_plot['Volume'], 
           color=['green' if c >= o else 'red' for c, o in zip(df_plot['Close'], df_plot['Open'])],
           width=0.0003, alpha=0.7)
    
    # Add titles and labels
    file_name = os.path.basename(file_path)
    ax1.set_title(f'Anchored VWAP - {file_name}\nAnchor: {anchor_date.strftime("%Y-%m-%d %H:%M")}', fontweight='bold')
    ...
    if debug_mode:
        txt = f"Debug Mode: ON\nCrossover Above: {len(crossover_above)}\nCrossover Below: {len(crossover_below)}"
    ...
    plot_path = f'anchored_vwap_chart_{timestamp}.png'
...
def print_alerts(df):
    """Print alert messages for crossovers"""
...
def main(file_path='SBIN_20250415.csv', year=2025, month=4, day=15, hour=9, minute=30, debug_mode=False):
    ...
    df, anchor_date = calculate_anchored_vwap(df, year, month, day, hour, minute)
    ...
    # Create chart
    create_advanced_chart(df, anchor_date, file_path, debug_mode)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Anchored VWAP Chart and Alert System')
    ...
    parser.add_argument('--minute', type=int, default=15, help='Anchor minute')
